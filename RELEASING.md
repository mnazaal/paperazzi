...
There is no changelog file — `git log` since 0.2.0 is the record. Release notes
are generated by `release.yml` from `git log <previous-tag>..<tag>`, so the commit
...
2. **build** — builds sdist + wheel, runs twine check, builds the Firefox +
   ...
   workflow artifact.
3. **github-release** — creates a GitHub Release with the git log since the
   previous tag as the release notes, attaches the sdist, wheel, and both extension zips
