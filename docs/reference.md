...
`pzi init --setup --bib ~/bibs/main.bib` writes `~/.config/pzi/config.toml` and an API token to `<data-home>/api_token` (mode 0600). Re-running `init` reuses the existing token so a paired browser extension keeps working; `--rotate-token` replaces it, which un-pairs the extension until you paste the new one in. `pzi add --force-new` inserts a second entry for a paper pzi considers a duplicate. Common options: `contact_email`, `unpaywall_email`, `browser_profile_path`, `browser_pdf_cmd`, `citekey_format`, `pdf_filename_format`, `papers_dir`, `browser_engine` (headless Playwright browser for automated capture, set in `config.toml`; `pzi init --browser` writes `browser_profile_path`, not this, and it is not the same setting as `PZI_BROWSER` below), and multiple `[[bibs]]`.

**`pdf_file_path_style` defaults to `"home"`**, so a `file = {...}` field is written `~/...` and a `.bib` shared between machines keeps resolving its PDFs. `"relative"` writes paths relative to the `.bib`, but only for PDFs stored under the `.bib`'s own directory — a library whose PDFs live elsewhere is unaffected by it. `"absolute"` is the old default and pins every path to one machine. To convert a library written under the old default, `pzi library reindex --convert-file-paths --dry-run` first: it reports what it would fold and what it would leave alone (paths outside `$HOME`, composite Zotero-style fields), and the applying run writes a `.convert-file-paths.bak` before touching anything.

**`pzi init --setup` does not write `browser_pdf_cmd`, deliberately.** It used to interpolate the interpreter it happened to be running under, which is an install-time snapshot of your environment written into a file you are invited to commit to your dotfiles — it went stale on a reinstall, a Python upgrade, a distribution rename, or the second machine, and a configured command *wins* over the one pzi builds at run time, so the stale value shadowed a working mechanism. pzi now builds the command from the running interpreter each time. Set `browser_pdf_cmd` yourself only to run a hook that is not pzi's own; to point pzi's hook at a particular browser profile, set `browser_profile_path` instead. A `browser_pdf_cmd` naming an interpreter that no longer exists is repaired in place, with a notice, when the rest of the command is pzi's own hook module. See `src/pzi/config.template.toml` for all options and comments. `papers_dir` defaults to `<bib-dir>/papers/`; use `_cmd` variants for secrets.
...
says which — there is no changelog file; it covered releases through 0.2.0 and
was deleted, and `git log` since then is the record. What is already true is
that each one has a test that fails when it changes, so a break is a decision
...
| `PZI_BROWSER_PROFILE` | Profile directory for that generated command, in place of the auto-detected default Firefox/Chrome profile. The `browser_profile_path` config key **wins** over this, matching `PZI_BROWSER_PDF_CMD` above and unlike `PZI_NODE` below. The browser is inferred from the profile path, so setting the config key also settles `PZI_BROWSER` |
