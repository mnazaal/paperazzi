...
**Twenty-one routes, not three.** An earlier version of this diagram showed
only the three capture routes, which understated what a caller holding the API
token can do. The count is six exact JSON `GET`s, two prefix `GET`s
(`/detail/<citekey>` and `/tags/<citekey>`), two binary `GET`s (`/pdf/<citekey>`
and `/export/raw`), and eleven `POST`s. Two deserve naming explicitly:
...
- pzi server binds `127.0.0.1` by default, and refuses to bind the wildcard
  address in any spelling — with or without a token.
...
|---|---|---|
| Bind address | `127.0.0.1` only | Keep loopback. A wildcard bind is **refused outright**, in every spelling — `0.0.0.0`, `::`, `*`, the empty string, and the legacy short forms `0`, `0.0`, `0x0`, `00` that `socket.bind` also accepts. The refusal holds even with a token: the Host check that guards against DNS rebinding has no bind address to match. Bind a specific LAN address if you must share, and only over a trusted network. |
| Auth token | **Mandatory.** Generated by `pzi init` into `<data-home>/api_token` (0600) and auto-read at runtime; the server refuses to start without one | Kept. To source it from a password manager instead, set `api_auth_token_cmd`. Copy the token into the extension popup. |
| `pzi server --no-auth` | Off | The one flag that turns the API's only authorization control off. It permits a *tokenless start*; it does not disable a token you already have. Every route — search, export, the PDFs, `capture`, `update`, `delete` — is then open to any process on the machine, so use it only for a throwaway library. It is a CLI flag on purpose: nothing reachable from `config.toml` or from the API itself can switch authentication off. |
...
- They can call the loopback API if the server was started with `--no-auth`.
  Since 0.1.0b5 it refuses to start without a token otherwise, so this is a
  state you have to opt into.
...
- The token, endpoint, preferences and capture history live in
  `chrome.storage.local`; `chrome.storage.session` holds only transient capture
  progress. (This once read the other way round — the token was described as
