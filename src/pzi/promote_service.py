...
from pzi.bib_repository import execute_write_plan, read_bib_file, update_bib_entry
...
_USER_OWNED_FIELDS = frozenset({"tags", "local_pdf_path", "citekey", "note"})
    ...
    for record in records:
            ...
            items.append(
                {
                    ...
                    "changed_fields": [],
                    "pdf_attached": None,
                    "note": f"low confidence ({score} < {confidence_threshold})",
                }
            )
            continue
        ...
        if duplicate_ck is not None:
            items.append(
                {
                    "preprint_citekey": preprint_ck,
                    "published_citekey": duplicate_ck,
                    "action": "skip",
                    "changed_fields": [],
                    "pdf_attached": None,
                    "note": f"already exists as {duplicate_ck}",
                }
            )
            continue
...
def _first_with_venue(results: list[dict[str, object]] | None) -> NormalizedRecord | None:
    if not results:
        return None
...
def _score_confidence(
    ...
    p_authors = {
        a.lower().strip()
        for a in (preprint.get("authors") or [])
        if isinstance(a, str)
    }
    c_authors = {
        a.lower().strip()
    ...
    score += min(len(p_authors & c_authors), 3)
...
def _handle_keep_preprint(
    *,
    ...
    published_ck = _generate_citekey_for_candidate(
        published, existing_citekeys
    )
    published["citekey"] = published_ck

    pdf_attached = False
    pdf_url = published.get("pdf_url")
    if isinstance(pdf_url, str) and pdf_url.strip() and not dry_run:
        path, _warn, _err = fetch_and_store_pdf_with_fallbacks(
            url=pdf_url,
            papers_dir=papers_dir,
            citekey=published_ck,
            fetch_binary=fetch_binary,
            flaresolverr_url=flaresolverr_url,
            browser_pdf_cmd=browser_pdf_cmd,
        )
        if path is not None:
            published["local_pdf_path"] = path
            pdf_attached = True

    cross_note_pp = f"Published version: {published_ck}"
    cross_note_pub = f"Preprint version: {preprint_ck}"

    changed_fields = sorted(
        key for key in published.keys() if published.get(key) != candidate.get(key)
    ) or ["venue", "doi"]
    ...
    if not dry_run:
        ...
        _add_note_to_citekey(bib_path, preprint_ck, cross_note_pp)
        _add_note_to_citekey(bib_path, published_ck, cross_note_pub)
...
def _handle_update_in_place(
    *,
    ...
    changed_fields = sorted(
        key for key in updated.keys() if updated.get(key) != preprint_record.get(key)
    )
...
def _merge_published_metadata(
    ...
    for key, value in candidate.items():
        if key in _USER_OWNED_FIELDS:
            continue
        merged[key] = value
    merged["tags"] = list(preprint.get("tags") or [])
    # Published version is a distinct publication; don't carry preprint identifiers
