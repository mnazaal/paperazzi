...
def test_search_with_no_matches_writes_nothing_to_stdout(tmp_path) -> None:
    """Empty results must not put a placeholder word into a pipeline.

    `pzi search ... | xargs pzi entries` would otherwise look up an entry named
    "no". The outcome is reported by the exit code and a stderr note instead.
    """
    def fake_search_bib(**kwargs):
        return {"status": "ok", "matches": [], "errors": []}
    ...
    assert stdout.getvalue() == ""
    assert "no matches" in stderr.getvalue()
