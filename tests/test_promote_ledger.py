...
def _state_with(citekey: str, checked_at: datetime, bib: str = "ml"):
    return promote_ledger.record_checked({}, bib, citekey, now=checked_at)


def test_a_freshly_recorded_citekey_is_recently_checked():
    state = _state_with("yeh-decoupled-2021", _NOW)

    assert promote_ledger.is_recently_checked(
...
def test_the_horizon_boundary_is_exclusive_on_both_sides():
    ...
    assert not promote_ledger.is_recently_checked(
        state, "ml", "yeh-decoupled-2021", now=_NOW, horizon_days=30
    )
...
def test_a_zero_horizon_disables_the_ledger():
    ...
    assert not promote_ledger.is_enabled(0)
    assert not promote_ledger.is_recently_checked(
...
def test_record_checked_does_not_mutate_its_input():
    ...
    promote_ledger.record_checked(state, "ml", "second-2021", now=_NOW)
...
def test_record_checked_keeps_other_bibs_and_other_citekeys():
    state = _state_with("first-2020", _NOW)
    state = promote_ledger.record_checked(state, "other", "third-2019", now=_NOW)

    state = promote_ledger.record_checked(state, "ml", "second-2021", now=_NOW)
...
def test_prune_drops_expired_entries_and_the_bibs_they_emptied():
    ...
    state = promote_ledger.record_checked(state, "ml", "fresh-2026", now=_NOW)
    state = promote_ledger.record_checked(
        state, "archive", "also-stale-2018", now=_NOW - timedelta(days=90)
    )

    pruned = promote_ledger.prune(state, now=_NOW, horizon_days=30)
...
def test_prune_leaves_state_alone_when_the_ledger_is_disabled():
    ...
    assert promote_ledger.prune(state, now=_NOW, horizon_days=0) == state


def test_a_round_trip_through_disk_preserves_the_decision(tmp_path):
    path = promote_ledger.ledger_path(tmp_path)
    state = _state_with("yeh-decoupled-2021", _NOW)

    promote_ledger.save(path, state)

    assert path.name == "promote-checked.json"
    assert promote_ledger.is_recently_checked(
        promote_ledger.load(path), "ml", "yeh-decoupled-2021",
        now=_NOW, horizon_days=30,
    )


def test_save_creates_the_data_home_directory(tmp_path):
    path = promote_ledger.ledger_path(tmp_path / "does" / "not" / "exist")

    promote_ledger.save(path, _state_with("yeh-decoupled-2021", _NOW))

    assert promote_ledger.load(path)["bibs"]["ml"]


def test_a_missing_ledger_reads_as_empty(tmp_path):
    assert promote_ledger.load(tmp_path / "nothing-here.json") == {}
...
def test_corrupt_or_foreign_state_reads_as_empty_rather_than_raising(tmp_path):
    ...
    assert promote_ledger.load(path) == {}
