...
def test_best_fuzzy_match_details_picks_best_match() -> None:
    records = [
        {
    ...
    matches = best_fuzzy_match_details(records, positions=[0])
    assert matches[0].citekey == "smith2024graph"


def test_best_fuzzy_match_details_rejects_title_match_with_no_shared_authors() -> None:
    ...
    assert best_fuzzy_match_details(records, positions=[0]) == {}


def test_best_fuzzy_match_details_accepts_strong_title_match_without_authors() -> None:
    ...
    matches = best_fuzzy_match_details(records, positions=[0])
    assert matches[0].citekey == "nguyen2024revisited"


def test_best_fuzzy_match_details_respects_year_window() -> None:
    records = [
        {"citekey": "q", "title": "Graph Parsers", "authors": ["Smith, Jane"], "year": 2024},
        {
...
def test_best_fuzzy_match_details_coerces_string_year() -> None:
...
def test_best_fuzzy_match_details_string_year_outside_window_excluded() -> None:
    records = [
        {"citekey": "q", "title": "Graph Parsers", "authors": ["Smith, Jane"], "year": "2024"},
        {
            "citekey": "smith2015graph",
...
def test_best_fuzzy_match_details_none_when_no_title() -> None:
    records = [
        {"title": None},
        {
            "citekey": "smith2024graph",
...
def test_best_fuzzy_match_details_only_scores_the_positions_it_was_given() -> None:
    records = [
    ...
    assert matches[0].citekey == "b"


def test_best_fuzzy_match_details_ignores_a_record_with_no_usable_title() -> None:
    records = [
        {"citekey": "a", "title": "", "authors": ["He, K"], "year": 2016},
        {"citekey": "b", "title": "Deep Residual Learning", "authors": ["He, K"], "year": 2016},
    ]

    assert best_fuzzy_match_details(records, positions=range(2)) == {}
